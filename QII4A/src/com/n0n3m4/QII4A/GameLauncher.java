...
public class GameLauncher extends Activity{		
	...
	final String default_gamedata="/sdcard/qii4a";
	...
	{
		if (!new File(dir+"/baseq2/config.cfg").exists()&&(new File(dir+"/baseq2/configpatched.qii4a").exists()))
			new File(dir+"/baseq2/configpatched.qii4a").delete();
		if (new File(dir+"/baseq2/config.cfg").exists()&&(!new File(dir+"/baseq2/configpatched.qii4a").exists()))
		{
			BufferedReader br=new BufferedReader(new FileReader(dir+"/baseq2/config.cfg"));
			String s;
			StringBuilder sb=new StringBuilder(0);
			while ((s=br.readLine())!=null)
			{
				if (!(s.contains("weapnext\"")||s.contains("weapprev\"")||
						s.contains("sensitivity")||s.contains("+moveleft\"")||s.contains("+moveright\"")
						||s.contains("gl_modulate")))
				sb.append(s+"\n");
			}
			br.close();
			sb.append("bind a \"+moveleft\"");sb.append('\n');
			sb.append("bind d \"+moveright\"");sb.append('\n');
			sb.append("bind MWHEELUP \"weapnext\"");sb.append('\n');
			sb.append("bind MWHEELDOWN \"weapprev\"");sb.append('\n');
			sb.append("set sensitivity \"8.000000\"");sb.append('\n');
			sb.append("set gl_modulate \"4\"");sb.append('\n');
			FileWriter fw=new FileWriter(dir+"/baseq2/config.cfg");
			fw.write(sb.toString());fw.flush();fw.close();
			fw=new FileWriter(dir+"/baseq2/configpatched.qii4a");
	...
	{					
		...
		q3ei.libname="libq2_neon.so";
		else
		q3ei.libname="libq2.so";	
	...
	{		
		...
		((EditText)findViewById(R.id.edt_cmdline)).setText(mPrefs.getString(Q3EUtils.pref_params, "yquake2.arm"));
