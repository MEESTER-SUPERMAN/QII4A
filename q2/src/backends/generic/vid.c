...
void (*AndroidEraseSwp_p)();
#include <stdbool.h>
bool (*AndroidGetSwp_p)();
...
VID_LoadRefresh(char *name)
{
	...
	path = "/""d""a""t""a""/""d""a""t""a""/""c""o""m"".""n""0""n""3""m""4"".""Q""I""I""4""A""/""l""i""b";//Cvar_Get("basedir", ".", CVAR_NOSET)->string;
	...
	AndroidEraseSwp_p=Sys_GetProcAddress(reflib_library, "AndroidEraseSwp");
	AndroidGetSwp_p=Sys_GetProcAddress(reflib_library, "AndroidGetSwp");
	(*AndroidSetResolution_p)(androw,androh);
...
#else
		#ifdef __ARM_NEON__
		sprintf(name, "ref_%s_neon.so", vid_ref->string);
		#else
